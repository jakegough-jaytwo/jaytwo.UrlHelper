...
public static class Url
{
    ...
    {
        var escapedArgs = formatArgs?.Select(x => Uri.EscapeDataString(x ?? string.Empty)).ToArray() ?? Array.Empty<string>();
    ...
    public static string Combine(string baseUrl, params string?[] segments)
    {
        ...
        {
            result = Combine(result, segment, throwOnInvalid: false) ?? string.Empty;
        }
